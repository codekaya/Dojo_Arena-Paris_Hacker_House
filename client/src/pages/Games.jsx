...
import { fetchAllGame, setGames } from '../../stores/games-store'
...
function GameList(props) {
  ...
  const { all_games: games_data } = useSelector((state) => state.games)
...
export default function Games() {
  ...
  const fetchGames = async () => {
    ...
    for (let i = 0; i < res.length; i++) {
      res[i].status = res[i].is_active ? (res[i].current_tour > 0 ? 'Ongoing' : 'Upcoming') : 'Past'
    }
